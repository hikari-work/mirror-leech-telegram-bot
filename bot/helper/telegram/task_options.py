...
from .message_utils import auto_delete_message, chat_of, edit_message, send_message
...
BOOL, VALUE, PRESET = "bool", "value", "preset"
...
class Option:
    ...
    kind: str
    """``bool``, ``value`` (needs typed text) or ``preset`` (one of the user's)."""
...
OPTIONS: dict[str, Option] = {
    ...
    "-c": Option("Copy preset", "copy_preset", PRESET),
    "-z": Option("Zip", "compress", BOOL),
    "-e": Option("Extract", "extract", BOOL, commands=("leech",)),
...
def render(
    ...
    for flag in options_for(command):
        if OPTIONS[flag].kind != PRESET:
            buttons.data_button(_flag_label(state, flag), _data(user_id, f"t:{flag}"))
    for name in _preset_names(state, presets):
        buttons.data_button(
            f"{_mark(state['-c'] == name)} {escape(name)}",
    ...
    return _text(state, presets, link), buttons.build_menu(2)
...
def _text(state: dict[str, Any], presets: dict[str, Any], link: str) -> str:
    ...
    if state["-c"]:
        ...
        lines.append(
            f"<b>Copying to preset:</b> <code>{escape(state['-c'])}</code> ({count})"
        )
    elif not presets:
        lines.append(
...
async def _close(query: CallbackQuery, prompt: Prompt, run: bool) -> None:
    ...
    if run:
        await edit_message(
            query.message, "<b>Starting…</b> The keyboard is closed."
        )
    else:
...
async def _ask_value(
    ...
    """Take the text behind ``-n`` or ``-sp`` from the user's next message."""
    spec = OPTIONS[flag]
    await query.answer()
    await edit_message(
        query.message,
        f"Send the <code>{escape(spec.label)}</code> for this task, or press a"
        f" button on the keyboard when you are done. Waiting {VALUE_TIMEOUT}s.",
    )
    value = await wait_for_reply(
        client, query.message, prompt.user_id, VALUE_TIMEOUT
    )
    if value is None:
        await _redraw(query.message, prompt, "No value arrived, so nothing changed.")
        return
    error = set_value(prompt.state, flag, value)
    note = error or f"{spec.label} set."
...
async def _redraw(message: Message, prompt: Prompt, note: str = "") -> None:
    ...
    text, buttons = render(
        prompt.state, prompt.presets, prompt.link, prompt.command, prompt.user_id
    )
    if note:
        text += f"\n\n{note}"
